create or replace function public.save_catalog_banner_campaign(
  p_banner_id uuid,
  p_promotion_id uuid,
  p_title text,
  p_slug text,
...
begin
  ...
  if p_theme not in ('wine', 'gold', 'forest') then
    raise exception 'Tema de banner invalido.';
  end if;
  ...
  insert into public.catalog_banners (
    id, promotion_id, eyebrow, title, subtitle, cta_label,
    image_url, mobile_image_url, image_alt, theme, sort_order,
